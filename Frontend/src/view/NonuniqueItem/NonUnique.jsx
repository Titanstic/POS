...
const NonUnique = () => {
  ...
  const [showStockBtn, setShowStockBtn] = useState(false);
  const [showStock, setShowStock] = useState(false);
  const [stockData, setStockData] = useState(null);

  useEffect(() => {
    if (data) {
      data.forEach(d => d.quantity === d.minQty && setShowStockBtn(true))
    }
  ...
  const checkStockLevel = () => {
    setShowStock(true);
    setStockData(data.filter(d => d.quantity <= d.minQty));
  }

  return (
    <div className="w-full bg-bg bg-opacity-20 font-poppins p-7 relative">
      <div className="flex justify-between">
        ...
        <div>
          {
            showStockBtn &&
            <button className="border text-left shadow-md rounded w-62 text-red-500 p-2 mr-5" onClick={() => checkStockLevel()}>
              Check Your Stock Level
              <br />
              Some products are need to restocking.
            </button>
          }
          <Link to={"/nonunique/add"}>
            <button className="bg-skin-fill p-2 text-xl rounded-md shadow-md">
              Add Product
            </button>
          </Link>
        </div>
              ...
              records?.map((item) => (
                ...
                >
                  ...
                  {/* {item.quantity === item.minQty ? (
                    <td className="  border shadow-md rounded right-48 w-62 text-red-500 cursor-pointer p-2" onClick={() => checkStockLevel()}>
      ...
      {/* Start Show stock data */}
      {
        showStock &&
        <>

          <div className="w-full h-full bg-gray-300 bg-opacity-70 absolute top-0 left-0 flex justify-center items-center ">
            <button className="absolute top-5 right-14 text-xl bg-red-500 border py-1 px-3" onClick={() => setShowStock(false)}>x</button>
            <div className="w-11/12 h-5/6 overflow-auto">
              <table className="w-full bg-white text-left table-auto mt-2 ">
                <thead className="bg-skin-fill bg-opacity-10 w-full">
                  <tr className="w-full flex px-2 ">
                    <th className="w-2/12 border border-white py-1 px-2">Code</th>
                    <th className="w-3/12 border border-white py-1 px-2">Image</th>
