...
const ExpenseExcelBtn = ({ expenseData, fileName }) => {
  const exportExcel = () => {
    const data = [];
    const date = new Date();
    ...
    data.push([
      `${firstDate?.toISOString().slice(0, 10)} to ${lastDate
        ?.toISOString()
        .slice(0, 10)}`,
    ]);
    data.push([""]);
    data.push([`Date and Time : ${fileName} - ${time}`]);
    const headers = ["ID", "Date", "Expense For", "Expense Amount"];
    data.push(headers);

    expenseData.forEach((resultItem) => {
      const row = [
        resultItem.id,
        resultItem.date,
        resultItem.expenseFor,
        resultItem.expenseAmount,
      ];
  ...
  return (
    <button
      className="bg-green-400 p-2 rounded font-poppins shadow"
      onClick={exportExcel}
    >
      Export To Excel
    </button>
...
export default ExpenseExcelBtn;
